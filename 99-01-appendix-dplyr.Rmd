...
We can add columns to our data frame, or edit existing columns, by using `mutate`. Here we'll add a column for home score differential. 
...
If we want to find the game with the smallest `diff` (the most negative `diff`), we could order the rows by `diff`. 
...
```{r}
d %>% arrange(desc(diff), desc(hscore))
```
