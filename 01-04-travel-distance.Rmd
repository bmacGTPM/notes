...
dd = dd %>% 
  ...
  left_join(ll, by=c('loc' = 'team'), suffix = c('.prev', ''))
title = "NBA Travel Routes"
g = ggplot(dd, aes(x=long, y=lat))+
  borders('state', colour=publightgray)+
  geom_segment(aes(xend=long.prev, yend=lat.prev), alpha=0.03)+
  geom_point(data=ll, color=pubred)+
  geom_text(data=ll, aes(label=team), hjust=rep(c(-0.1, 1.1), 15))+
  labs(title    = title)+
...
dg = dd %>%
  group_by(prev.loc, loc, lat, long, lat.prev, long.prev) %>%
  summarise(Trips = n())
title = "NBA Travel Routes Weighted By Number of Trips"
g = ggplot(dg, aes(x=long, y=lat))+
  borders('state', colour=publightgray)+
  geom_segment(aes(xend=long.prev, yend=lat.prev, size=Trips), alpha=0.3)+
  geom_point(data=ll, color=pubred)+
  geom_text(data=ll, aes(label=team), hjust=rep(c(-0.1, 1.1), 15))+
  labs(title = title, 
       size  = 'Trips')+
  ...
  theme_pub(type='map', base_size = 36/3)+
  theme(legend.title = element_text(size=36/3, vjust=0.5,
                                    margin = margin(0, 0, 0, 150/72/3, 'in')) )
g
```

## Travel matrix

Number of times that teams travel from one city to another during the season. 

```{r}
dr = dg %>%
  ungroup() %>%
  filter(prev.loc != loc) %>%
  complete(prev.loc, loc, fill=list(Trips=0)) %>%
  left_join(tms, by=c('prev.loc' = 'team')) %>%
  left_join(tms, by=c('loc'      = 'team'), suffix = c('.prev', '')) %>%
...
g = ggplot(dr, aes(x=loc, y=prev.loc, fill=Trips))+
  geom_tile(linewidth=0.4, show.legend = T, color=pubdarkgray) + 
  facet_grid(div.prev ~ div, scales='free')+ ## now faceting by division
  scale_fill_gradient(low = pubbackgray,
                      high = pubred,
                      na.value = 'white',
                      oob=squish, 
...
Note that this is not the same as the schedule matrix, which shows games played between each pair of teams. In
